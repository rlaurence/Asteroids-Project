...
public class Polygon {
	...
	public void paint(Graphics brush){
		...
		int[] xcoord=new int[newPoints.length];
		int[] ycoord=new int[newPoints.length];
		
		for(int i = 0; i< newPoints.length; i++){
			xcoord[i] = (int)newPoints[i].getX();
			ycoord[i] = (int)newPoints[i].getY();
		}
	    brush.drawPolygon (xcoord, ycoord, xcoord.length); 
	}
